...
class Tag extends Component {
  ...
  render = () => {
    ...
    let textColor = colorHasher.hsl(label)[2] > 0.5;

    const tagComponent = (
      <span style={{opacity: isDragging ? 0 : 1, background: colorHasher.hex(label)}} className={props.classNames.tag+' '+(textColor ? 'black' : 'white')}>
        {label}
